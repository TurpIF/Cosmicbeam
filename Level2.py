...
class Level2(Game.Game):
	def construct(self):
		...
		self.emitters = []
		self.emitters += [Emitter(self.sprEmitters[0].GetPosition()[0] + self.sprEmitters[0].GetSize()[0] - 5, self.sprEmitters[0].GetPosition()[1] + self.sprEmitters[0].GetSize()[1] / 2.0, 260, 40, sf.Color(255, 0, 0), 500)]
		
		#LOADING AND PLACING RECEPTOR
		img = sf.Image()
		img.LoadFromFile("./img/receptor.png")
		self.sprReceptors += [sf.Sprite(img)]
		self.sprReceptors[-1].SetPosition(self.max_width - self.sprEmitters[0].GetSize()[0], 50)
		
		self.receptors = []
		self.receptors += [Receptor(self.sprReceptors[0], 200)]
